...
private:
	...
	void initVulkan() {
		...
		loadPrimitive("wire", "models/wire2.obj", glm::vec3(0.7, 0, 0));
	...
	void drawBlocks() {
		...
		for (int i = 0; i < blocks.blocks.size(); i++) {
			...
			bool y = false;
			bool z = false;
			glm::vec3 stateColour;
			if (blocks.blocks[i]->state == true) {
				stateColour = glm::vec3(0.5, 0.5, 0.5);
			}
			else {
				stateColour = glm::vec3(0, 0, 0);
			}
			...
			case wire:
				...
				if (blocks.doesBlockExist(blocks.blocks[i]->position.x, blocks.blocks[i]->position.y - 1, blocks.blocks[i]->position.z)) {
					addPrimitive("wire", blocks.blocks[i]->position, NY, stateColour);
					y = true;
				}
				//top Y back
				if (blocks.doesBlockExist(blocks.blocks[i]->position.x, blocks.blocks[i]->position.y + 1, blocks.blocks[i]->position.z)) {
					addPrimitive("wire", blocks.blocks[i]->position, PY, stateColour);
					y = true;
				}
				//Z front
				if (blocks.doesBlockExist(blocks.blocks[i]->position.x, blocks.blocks[i]->position.y, blocks.blocks[i]->position.z - 1)) {
					addPrimitive("wire", blocks.blocks[i]->position, NZ, stateColour);
					z = true;
				}
				//Z back
				if (blocks.doesBlockExist(blocks.blocks[i]->position.x, blocks.blocks[i]->position.y, blocks.blocks[i]->position.z + 1)) {
					addPrimitive("wire", blocks.blocks[i]->position, PZ, stateColour);
					z = true;
				}
				//X front
				if (blocks.doesBlockExist(blocks.blocks[i]->position.x - 1, blocks.blocks[i]->position.y, blocks.blocks[i]->position.z)) {
					addPrimitive("wire", blocks.blocks[i]->position, NX, stateColour);
					x = true;
				}
				//X back
				if (blocks.doesBlockExist(blocks.blocks[i]->position.x + 1, blocks.blocks[i]->position.y, blocks.blocks[i]->position.z)) {
					addPrimitive("wire", blocks.blocks[i]->position, PX, stateColour);
					x = true;
				}
				if ( !x && !y && !z) {
					addPrimitive("wire_center", blocks.blocks[i]->position, stateColour);
			...
			case inverter:
				...
				case positiveX:
					addPrimitive("inverter", blocks.blocks[i]->position, NX, stateColour);
					break;
				case negativeX:
					addPrimitive("inverter", blocks.blocks[i]->position, PX, stateColour);
					break;
				case positiveY:
					addPrimitive("inverter", blocks.blocks[i]->position, NY, stateColour);
					break;
				case negativeY:
					addPrimitive("inverter", blocks.blocks[i]->position, PY, stateColour);
					break;
				case positiveZ:
					addPrimitive("inverter", blocks.blocks[i]->position, NZ, stateColour);
					break;
				case negativeZ:
					addPrimitive("inverter", blocks.blocks[i]->position, PZ, stateColour);
					break;
			...
			case andGate:
				switch (blocks.blocks[i]->direction) {
				case positiveX:
					addPrimitive("andGate", blocks.blocks[i]->position, NX, stateColour);
					break;
				case negativeX:
					addPrimitive("andGate", blocks.blocks[i]->position, PX, stateColour);
					break;
				case positiveY:
					addPrimitive("andGate", blocks.blocks[i]->position, NY, stateColour);
					break;
				case negativeY:
					addPrimitive("andGate", blocks.blocks[i]->position, PY, stateColour);
					break;
				case positiveZ:
					addPrimitive("andGate", blocks.blocks[i]->position, NZ, stateColour);
					break;
				case negativeZ:
					addPrimitive("andGate", blocks.blocks[i]->position, PZ, stateColour);
					break;
			...
			case orGate:
				switch (blocks.blocks[i]->direction) {
				case positiveX:
					addPrimitive("orGate", blocks.blocks[i]->position, NX, stateColour);
					break;
				case negativeX:
					addPrimitive("orGate", blocks.blocks[i]->position, PX, stateColour);
					break;
				case positiveY:
					addPrimitive("orGate", blocks.blocks[i]->position, NY, stateColour);
					break;
				case negativeY:
					addPrimitive("orGate", blocks.blocks[i]->position, PY, stateColour);
					break;
				case positiveZ:
					addPrimitive("orGate", blocks.blocks[i]->position, NZ, stateColour);
					break;
				case negativeZ:
					addPrimitive("orGate", blocks.blocks[i]->position, PZ, stateColour);
					break;
			...
			case xorGate:
				switch (blocks.blocks[i]->direction) {
				case positiveX:
					addPrimitive("xorGate", blocks.blocks[i]->position, NX, stateColour);
					break;
				case negativeX:
					addPrimitive("xorGate", blocks.blocks[i]->position, PX, stateColour);
					break;
				case positiveY:
					addPrimitive("xorGate", blocks.blocks[i]->position, NY, stateColour);
					break;
				case negativeY:
					addPrimitive("xorGate", blocks.blocks[i]->position, PY, stateColour);
					break;
				case positiveZ:
					addPrimitive("xorGate", blocks.blocks[i]->position, NZ, stateColour);
					break;
				case negativeZ:
					addPrimitive("xorGate", blocks.blocks[i]->position, PZ, stateColour);
					break;
			...
			if (drawBoxes) {
				...
				Vertex temp = {};
				temp.texCoord = { 0, 0 };
				temp.colour = { color.r + 0.3, color.g + 0.3, color.b + 0.3 };
				temp.pos = blocks.blocks[i]->position + glm::vec3(0, 0, 0);
				vertices.push_back(temp);
				temp.colour = { color.r, color.g, color.b };
	...
	void addPrimitive(std::string name, glm::vec3 offset, orientation _orientation, glm::vec3 _colourOffset) {
		for (int i = 0; i < primitives.size(); i++) {
			...
			if (_colourOffset != glm::vec3(0, 0, 0)) {
				for (int i = 0; i < vector->size(); i++) {
					vector->at(i).colour += _colourOffset;
				}
			}
	...
	void addPrimitive(std::string name, glm::vec3 offset, glm::vec3 _colourOffset) {
		addPrimitive(name, offset, PX, _colourOffset);
	}
	...
	void loadModel(std::vector<Vertex>* _vertices, std::vector<uint32_t>* _indices, std::string path, glm::vec3 colour) {
		...
		for (const auto& shape : shapes) {
			for (const auto& index : shape.mesh.indices) {
				...
				vertex.colour = { colour.r -0.2 + 0.4*((double)rand() / (RAND_MAX)), colour.g - 0.2 + 0.4*((double)rand() / (RAND_MAX)), colour.b - 0.2 + 0.4*((double)rand() / (RAND_MAX)) };
	...
	void mainLoop() {
		while (!glfwWindowShouldClose(window)) {
			...
			if (keys.f) {
				...
				std::cout << cameraMin.x << "," << cameraMin.y - 1 << "," << cameraMin.z << std::endl;
			}
